...
let newArray = array.map(item =>
  item % 2 ? { digit: item, odd: true } : { digit: item, odd: false }
);
...
function isSortArray(products, min, max) {
  let prow;
  prow = products.filter(item => item.price > min && item.price < max);
  prow.sort((next, prev) => next.price - prev.price);
  return console.log(prow);
}

isSortArray(products, 20, 200);
